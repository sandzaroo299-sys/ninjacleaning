...
from app.models import Apartment, Building, ComplaintPhoto, Notification, Request, Setting, User, WorkLog
from app.config import settings
from app.api.schemas import (
    ApartmentOut, BuildingOut, ComplaintCreate, ComplaintOut, NotificationOut,
    RegisterRequest, RegisterResponse, RequestCreate, RequestOut, RequestStatusUpdate,
    SettingOut, SettingUpdate, SolarPositionOut, UserOut, WeatherAlertOut,
)
...
def register(data: RegisterRequest, telegram_id: int = Query(...), db: Session = Depends(get_db)):
    user = get_user_by_telegram(db, telegram_id)
    if user:
        if user.role in ("worker", "admin"):
            raise HTTPException(status_code=403, detail="Сотрудники не могут регистрироваться как жильцы")
    else:
        user = User(telegram_id=telegram_id, full_name=data.full_name, role="resident")
        db.add(user)
        db.flush()
    ...
    return RegisterResponse(user=UserOut.from_orm(user), apartment=ApartmentOut.from_orm(apartment))

@router.get("/buildings", response_model=List[BuildingOut])
def get_buildings(db: Session = Depends(get_db)):
    return [BuildingOut.from_orm(b) for b in db.query(Building).all()]

@router.get("/apartments/by_number", response_model=ApartmentOut)
def get_apartment_by_number(building_id: int, apartment_number: str, db: Session = Depends(get_db)):
    apartment = (
        db.query(Apartment)
...
def create_request(data: RequestCreate, telegram_id: int = Query(...), db: Session = Depends(get_db)):
    user = get_user_or_404(db, telegram_id)
    ...
    log = WorkLog(request_id=request.id, user_id=user.id, action="created", details=f"Заявка создана: {data.service_type}")
    db.add(log)
...
def get_my_requests(telegram_id: int = Query(...), db: Session = Depends(get_db)):
    user = get_user_or_404(db, telegram_id)
...
def get_request(request_id: int, telegram_id: int = Query(...), db: Session = Depends(get_db)):
    user = get_user_or_404(db, telegram_id)
...
def update_request_status(request_id: int, data: RequestStatusUpdate, telegram_id: int = Query(...), db: Session = Depends(get_db)):
    user = get_user_or_404(db, telegram_id)
    check_role(user, ["worker", "admin"])
...
def create_complaint(request_id: int, data: ComplaintCreate, telegram_id: int = Query(...), db: Session = Depends(get_db)):
    user = get_user_or_404(db, telegram_id)
    if user.role != "resident":
...
def get_my_notifications(telegram_id: int = Query(...), only_unread: bool = False, db: Session = Depends(get_db)):
    user = get_user_or_404(db, telegram_id)
...
def get_setting(key: str, telegram_id: int = Query(...), db: Session = Depends(get_db)):
    user = get_user_or_404(db, telegram_id)
...
def create_or_update_setting(data: SettingUpdate, telegram_id: int = Query(...), db: Session = Depends(get_db)):
    user = get_user_or_404(db, telegram_id)
...
def get_weather_alert(building_id: int, telegram_id: int = Query(...), db: Session = Depends(get_db)):
    user = get_user_or_404(db, telegram_id)
    building = db.query(Building).filter(Building.id == building_id).first()
    if not building:
        raise HTTPException(status_code=404, detail="Дом не найден")
    return get_weather_alert_for_building(building)

@router.get("/solar/position", response_model=SolarPositionOut)
def get_solar_position(building_id: int, telegram_id: int = Query(...), db: Session = Depends(get_db)):
    user = get_user_or_404(db, telegram_id)
    building = db.query(Building).filter(Building.id == building_id).first()
    if not building:
        raise HTTPException(status_code=404, detail="Дом не найден")
    return get_solar_position_for_building(building)

@router.get("/qr/{building_id}")
def get_qr_code(building_id: int, telegram_id: int = Query(...), db: Session = Depends(get_db)):
    user = get_user_or_404(db, telegram_id)
    check_role(user, ["admin"])
    building = db.query(Building).filter(Building.id == building_id).first()
    if not building:
...
def export_requests_csv(telegram_id: int = Query(...), status: Optional[str] = None, db: Session = Depends(get_db)):
    user = get_user_or_404(db, telegram_id)
    check_role(user, ["admin"])
